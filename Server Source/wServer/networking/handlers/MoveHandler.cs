    ...
    internal class MoveHandler : PacketHandlerBase<MovePacket>
    {
        ...
        protected override void HandlePacket(Client client, MovePacket packet)
        {
            client.Manager.Logic.AddPendingAction(t =>
            {
                ...
                /*if (!client.Player.HasConditionEffect(ConditionEffectIndex.Speedy) && (client.Character.Speed > client.Character.Speed * ((0.5f + client.Player.StatsManager.GetStats(4) / 75F * (2 - 0.5f)))))
                {
                    log.Error(client.Player.Name + " is running too fast. Ignored");
                    return;
                } */
