...
namespace wServer.realm
{
    public class ChatManager
    {
        ...
        public void Say(Player src, string text)
        {
            ...
            {
                Name = (src.Client.Account.Rank >= 10 ? "_" : src.Client.Account.Rank >= 8 ? "|" : src.Client.Account.Rank >= 7 ? "[" : src.Client.Account.Rank >= 5 ? "]" : src.Client.Account.Rank >= 1 ? "%" : "") + src.Name,
                ObjectId = src.Id,
                Stars = src.Stars,
                BubbleTime = 10,
                Recipient = "",
        ...
        public void IngameAnnounce(string text)
        {
            foreach (var i in manager.Clients.Values)
                i.SendPacket(new TextPacket()
                {
