...
public class ActivateEffect
{
    public ActivateEffect(XElement elem)
    {
        ...
        if (elem.Attribute("PetSkinType") != null)
            PetSkinType = int.Parse(elem.Attribute("PetSkinType").Value);

        if (elem.Attribute("MobId") != null)
            MobId = (elem.Attribute("MobId").Value);

        if (elem.Attribute("FameAmount") != null)
            FameAmount = int.Parse(elem.Attribute("FameAmount").Value);

        if (elem.Attribute("GoldAmount") != null)
            GoldAmount = int.Parse(elem.Attribute("GoldAmount").Value);
    ...
    public int PetSkinType { get; private set; }
    public string MobId { get; private set; }
    public int FameAmount { get; private set; }
...
public class Item : IFeedable
{
    ...
    public Item(ushort type, XElement elem)
    {
        try
        {
            ...
            SellPrice = (n = elem.Element("SellPrice")) != null ? Utils.FromString(n.Value) : 0; //sell command
