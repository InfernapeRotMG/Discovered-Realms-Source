...
namespace wServer.realm.entities.player
{
    partial class Player
    {
        ...
        public bool HeyGuys(Player player, RealmTime time, Item item, UseItemPacket pkt, int SkinId)
        {
            int PetSkin = 0;
            switch (PetSkin)
            {
                case 0:
                    {
                        player.Manager.Database.DoActionAsync(db =>
                        {
                            ...
                            var AccIds = player.AccountId;
                            var PetIds = player.Pet.PetId;

                            var cmd = db.CreateQuery();
                            cmd.CommandText = "UPDATE pets SET skin=@changeskin WHERE accId=@accId AND petId=@petId";
                            cmd.Parameters.AddWithValue("@changeskin", SkinId);
                            cmd.Parameters.AddWithValue("@petId", PetIds);
                            cmd.Parameters.AddWithValue("@accId", AccIds);
        ...
        public bool Activate(RealmTime time, Item item, UseItemPacket pkt)
        {
            ...
            foreach (ActivateEffect eff in item.ActivateEffects)
            {
                switch (eff.Effect)
                {
                    ...
                    case ActivateEffects.Bagston:
                        {
                            int PetSkin = Random.Next(0, 0);
                            switch (PetSkin)
                            {
                                case 0:
                                    {
                                        Manager.Database.DoActionAsync(db =>
                                        {
                                            var AccIds = AccountId;
                                            var PetIds = Pet.PetId;
                                            var cmd = db.CreateQuery();
                                            cmd.CommandText = "UPDATE pets SET skin=@changeskin WHERE accId=@accId AND petId=@petId";
                                            cmd.Parameters.AddWithValue("@changeskin", item.ActivateEffects[0].PetSkinType);
                                            cmd.Parameters.AddWithValue("@petId", PetIds);
                                            cmd.Parameters.AddWithValue("@accId", AccIds);
                                            cmd.ExecuteNonQuery();
                                            SendInfo("Successfully Changed Pet Skin, rejoin the server to make it appear.");
                                        });
                                    }
                                    break;
                    ...
                    case ActivateEffects.AddFame:
                        {
                            int AddFameMayn = Random.Next(0, 0);
                            switch (AddFameMayn)
                            {
                                case 0:
                                    {
                                        Manager.Database.DoActionAsync(db =>
                                        {
                                            CurrentFame = Client.Account.Stats.Fame = db.UpdateFame(Client.Account, +item.ActivateEffects[0].FameAmount);
                                            UpdateCount++;
                                        });
                                    }
                                    break;
                    ...
                    case ActivateEffects.AddGold:
                        {
                            int AddGoldMayn = Random.Next(0, 0);
                            switch (AddGoldMayn)
                            {
                                case 0:
                                    {
                                        Manager.Database.DoActionAsync(db =>
                                        {
                                            var AccIds = AccountId;
                                            var cmd = db.CreateQuery();
                                            cmd.CommandText = "UPDATE stats SET credits=@amount WHERE accId=@accId";
                                            cmd.Parameters.AddWithValue("@amount", item.ActivateEffects[0].GoldAmount);
                                            cmd.Parameters.AddWithValue("@accId", AccIds);
                                            cmd.ExecuteNonQuery();
                                            UpdateCount++;
                                        });
