...
namespace wServer.networking.handlers
{
    internal class HelloHandler : PacketHandlerBase<HelloPacket>
    {
        ...
        {
            string longIP = client.Socket.RemoteEndPoint.ToString();
            ...
            client.Manager.Database.DoActionAsync(db =>
            {
                ...
                if (!client.Account.NameChosen)
                {
                    ...
                    {
                        ErrorId = 0,
                        ErrorDescription = "Choose name before joining."
                ...
                if (!client.Account.IsGuestAccount)
                {
                    ...
                    if (db.IpExists(usrIP[0]))
                        if (db.IsIpBanned(usrIP[0]))
                        {
                            log.Warn(@"The ip " + usrIP[0] + " is banned, and trying to connect.");
                            client.SendPacket(new FailurePacket
                            {
                                ErrorId = 0,
                                ErrorDescription = "You are IP Banned.",
                            });
                            client.Disconnect();
                            return;
                        }
                ...
                else
                {
                    ...
                    db.AddIp(usrIP[0]);
                    db.UpdateIp(usrIP[0], client.Account.AccountId);
