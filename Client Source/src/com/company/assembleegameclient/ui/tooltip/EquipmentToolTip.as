...
import kabam.rotmg.assets.EmbeddedAssets_Whitebag;
...
public class EquipmentToolTip extends ToolTip {
    ...
    private var WhitebagImage:Bitmap;
    ...
    private var SellText:TextFieldDisplayConcrete;
    ...
    private function addWhitebag():void { //Makes a Whitebag icon besides the Whitebag Drops.
        if (this.objectXML.hasOwnProperty("WhiteBag")) {
            this.WhitebagImage = new EmbeddedAssets_Whitebag();
            this.WhitebagImage.filters = [new DropShadowFilter(0, 0, 0, 0.5, 12, 12)];
            addChild(this.WhitebagImage);
        }
    }


    private function makesellprice():void{
        var _local_1:int;
        if (this.objectXML.hasOwnProperty("SellPrice")) {
            _local_1 = ((((this.playerCanUse) || ((this.player == null)))) ? 0xFFFFFF : 16549442);
            this.SellText = new TextFieldDisplayConcrete().setSize(12).setColor(_local_1).setBold(true).setTextWidth((((MAX_WIDTH - this.icon.width) - 4) - 30)).setWordWrap(true);
            this.SellText.setStringBuilder(new StaticStringBuilder().setString(("Sell Price: " + this.objectXML.SellPrice)));
            this.SellText.filters = [new DropShadowFilter(0, 0, 0, 0.5, 12, 12)];
            waiter.push(this.SellText.textChanged);
            addChild(this.SellText);
        }
    }
    ...
    override protected function alignUI():void {
        ...
        if (this.powerText) {
            ...
            if (this.SellText) {
                if (contains(this.SellText)) {
                    this.SellText.x = 4;
                    this.SellText.y = _local_1 + 12;
                }
            }
            if (this.WhitebagImage) {
                if (contains(this.WhitebagImage)) {
                    this.WhitebagImage.x = 185;
                    this.WhitebagImage.y = 12;
                }
            }
        }
    }
